...
//include passport package
var passport = require('passport');
var session = require('express-session');
var flash = require('connect-flash');
var localStrategy = require('passport-local').Strategy;

//initialize the passport package for authentication'
app.use(flash());
...
passport.use(new facebookStrategy({
    ...
    function (accessToken, refreshToken, profile, cb) {// cb is callback
      // what to do when fb return profile

      // check if this fb profile is already in our accounts
      Account.findOne({oauthID: profile.id }, function (err, user){
        ...
        if(err)
        {
          ...
          // if the user already exits , continue
          if(user !== null)
          {
            // cb has the first parameter is erro in this case there is no error because we found the user so null
            cb(null, user);
          }
          else
          {
            // valid fb user but not in mongo db yet so add the user in the mongo db collection
            user = new Account(
                {
                  // this is what comes back to us
                  oauthID: profile.id,
